...
public class MinecraftInstaller : InstallerBase
{
    ...
    public MinecraftInstaller(string gameId, string root = ".minecraft", Action<string, int>? onProgressChanged = null,
        Action<string>? onSpeedChanged = null)
    {
        Root = root;
        GameId = gameId;
    ...
    /// <returns></returns>
    public async Task<InstallResult> InstallAsync(string? gameCoreName = null, bool mandatory = false,
        CancellationToken cancellationToken = default)
    {
            ...
            else
            {
                if (mandatory)
                {
                    try
                    {
                        ...
                        var gameCoreData =
                            JsonSerializer.Deserialize<Dictionary<string, object>>(gameCoreJson, options);
        ...
        try
        {
            ...
            foreach (var versionDownload in versionEntity.Libraries)
                if (versionDownload?.Downloads != null)
                    if (ShouldIncludeLibrary(versionDownload.Rule))
                    {
                        ...
                        {
                            var basePath = MinecraftInstallerModel.BuildFromName(versionDownload.Name,
                            ...
                            {
                                if (!HashUtil.VerifyFileHash(jarFilePath, versionDownload.Downloads.Artifact.Sha1,
                                        SHA1.Create()))
                                    downloadList.Add(new DownloadItem(jarDownloadPath, jarFilePath));
                            }
                        ...
                        {
                            var nativeTemplate = versionDownload.Natives["windows"];
                            var native = nativeTemplate.Replace("${arch}",
                                SystemUtil.GetOperatingSystemBit().ToString());

                            var nativesWindows = versionDownload.Downloads.Classifiers[native];

                            var jarFilePathClassifiers = Path.Combine(GamePath, "libraries",
                                nativesWindows.Path.Replace("/", Path.DirectorySeparatorChar.ToString()));
                            ...
                            {
                                if (!HashUtil.VerifyFileHash(jarFilePathClassifiers, nativesWindows.Sha1,
                                        SHA1.Create()))
                                    downloadList.Add(new DownloadItem(jarDownloadPathClassifiers,
                                        jarFilePathClassifiers));
                            }
            ...
            librariesDownloader.OnSpeedChanged = speed => { OnSpeedChanged?.Invoke(CalcMemoryMensurableUnit(speed)); };
        ...
        try
        {
            ...
            if (!FileUtil.IsDirectory(assetsIndex, true) || !FileUtil.IsFile(jsonPath))
            {
                ...
                var formattedJson =
                    JsonSerializer.Serialize(JsonSerializer.Deserialize<object>(assetsJsonContent), options);
            ...
            if (assetsInfo != null && assetsInfo.Objects != null)
                ...
                {
                    var baseAssetsPath = $"{kvp.Value.Hash.Substring(0, 2)}/{kvp.Value.Hash}";
                    var downloadUrl = DownloadAPIs.Current.Assets + "/" + baseAssetsPath;
                    var localPath = Path.Combine(GamePath, "assets", "objects",
                        baseAssetsPath.Replace("/", Path.DirectorySeparatorChar.ToString()));
